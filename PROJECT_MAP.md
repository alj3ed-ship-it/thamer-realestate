## آلية حساب "صافي الإيراد السنوي" مقابل "إجمالي قيمة العقود" (موثّق 2026-08-09)

### السؤال اللي بدأ التوثيق
ليش رقم "إجمالي قيمة المحفظة العقارية" في صفحة تدقيق البيانات (2,061,046 ريال)
يختلف عن "صافي الإيراد السنوي" في لوحة التحكم الرئيسية (1,916,292 ريال)؟

**الفرق = 144,754 ريال — وهو صحيح ومقصود، مو خطأ بالنظام.**

### السبب: نوعين من عقود الضريبة (VAT) حسب من يتحمّلها

| نوع العقد | `tax_enabled` | `amount_includes_vat` | من يدفع الضريبة | معادلة الصافي |
|---|---|---|---|---|
| **شامل الضريبة** | true | true | المستأجر (تُخصم من الإيجار وتُسدَّد للهيئة) | `الصافي = قيمة العقد ÷ 1.15` |
| **غير شامل الضريبة** | true | false | المالك (من جيبه، بعد استلام كامل المبلغ) | `الصافي = قيمة العقد × 0.85` |
| **بدون ضريبة أصلاً** | false | false | لا يوجد | `الصافي = قيمة العقد` (بدون تغيير) |

### التطبيق الفعلي على المحفظة (4 عقارات فقط من أصل 8 خاضعة للضريبة)

| العقار | قيمة العقد | نوع الضريبة | الصافي الفعلي | مبلغ الخصم |
|---|---|---|---|---|
| عمارة المجاهدين | 862,500 | شامل (÷1.15) | 750,000 | 112,500 |
| أرض اسفل الجبل | 150,026 | غير شامل (×0.85) | 127,522 | 22,504 |
| أرض حماد | 35,000 | غير شامل (×0.85) | 29,750 | 5,250 |
| عمارة سلمان (محل ضريبي واحد فقط) | 30,000 | غير شامل (×0.85) | — | 4,500 |
| باقي 6 عقارات (بدون ضريبة) | 1,713,520 | — | 1,713,520 | 0 |

**مجموع الخصومات: 112,500 + 22,504 + 5,250 + 4,500 = 144,754 ريال** ✅
يطابق تماماً الفرق بين (2,061,046) و(1,916,292).

### قاعدة مهمة يجب تذكرها مستقبلاً
- الضريبة تُحسب على **كامل مبلغ العقد** بغض النظر عن تاريخ التفعيل (`tax_effective_hijri`) — لا يوجد تناسب زمني (proration) في حساب لوحة التحكم.
- التحقق من هذي الآلية يكون عبر استعلام مباشر على جدول `leases`:
```sql
SELECT p.name, l.contract_value, l.tax_enabled, l.amount_includes_vat
FROM leases l JOIN properties p ON p.id = l.property_id
WHERE l.status = 'active';
```
- تم التحقق والتوثيق بتاريخ 2026-08-09 عبر اتصال Supabase MCP المباشر (project_id: rmdwemrmtfcwvuxwskid).# خريطة مشروع "مكتب ثامر بن سلمان العقاري"
