...
# ---------------------------------------------------------------
# 1) التراجع عن دوال الحساب الميلادي والرجوع للحساب الهجري الأصلي
# ---------------------------------------------------------------
...
assert new1 in content, "❌ الملف مو بنسخته المحدَّثة (الميلادية) — تأكد إنك ما طبّقت التراجع من قبل."
content = content.replace(new1, old1)

# ---------------------------------------------------------------
# 2) إزالة start_date من استعلام العقود (رجوع لما كان)
# ---------------------------------------------------------------
new2 = '''    leases (
...
assert new2 in content, "❌ لم يتم العثور على استعلام leases المحدَّث (new2)."
content = content.replace(new2, old2)

# ---------------------------------------------------------------
# 3) الرجوع لـ computeStatus الأصلية (تستقبل كائن هجري)
# ---------------------------------------------------------------
...
assert new3 in content, "❌ لم يتم العثور على دالة computeStatus المحدَّثة (new3)."
content = content.replace(new3, old3)

# ---------------------------------------------------------------
# 4) الرجوع لحلقة البحث الأصلية (فلترة هجرية مباشرة)
# ---------------------------------------------------------------
...
assert new4 in content, "❌ لم يتم العثور على شرط الفلترة المحدَّث (new4)."
content = content.replace(new4, old4)
...
assert new5 in content, "❌ لم يتم العثور على كتلة status/dueDateHijri المحدَّثة (new5)."
content = content.replace(new5, old5)
...
print("✅ تم التراجع — src/Entitlements.jsx رجعت لحالتها الأصلية (الهجري كأساس).")
print("   - تاريخ الاستحقاق يُحسب الآن من start_date_hijri (زي ما كان قبل تحديث اليوم)")
