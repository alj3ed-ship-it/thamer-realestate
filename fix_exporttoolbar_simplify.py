# -*- coding: utf-8 -*-
"""
Patch: تبسيط منطق تلوين ExportToolbar.jsx — بس عمود "المتبقي" يطلع أحمر
(تنبيه)، وباقي الأعمدة عادية. + إضافة خلفية فاتحة لصفوف "متبقي جزء"
بالجدول المجمَّع (بدل تلوين كل رقم بداخلها لحاله).

الاستخدام:
    cd C:\\Users\\aljuaid\\Desktop\\thamer-realestate
    python fix_exporttoolbar_simplify.py
"""
import pathlib
...
"""  const amountFontColor = (col) => {
    if (col.key === "amountDisplay") return "#e74c3c";
  ...
  const amountFontColorArgb = (col) => {
    if (col.key === "amountDisplay") return "FFE74C3C";
