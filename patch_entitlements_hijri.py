...
# ============================================
old_h2g = '''// تحويل هجري إلى ميلادي (نفس الخوارزمية المستخدمة في صفحة الدفعات)
...
new_h2g = '''// تحويل هجري إلى ميلادي باستخدام تقويم "أم القرى" الرسمي (بحث تكراري عبر Intl، نفس الأسلوب في VatReturns.jsx)
function hijriToGregorian(hy, hm, hd) {
  try {
...
new_g2h = '''// تحويل ميلادي إلى هجري باستخدام تقويم "أم القرى" الرسمي (عبر Intl، نفس الأسلوب في VatReturns.jsx)
function gregorianToHijri(dateStr) {
  if (!dateStr) return null;
  const d = new Date(dateStr);
  if (isNaN(d.getTime())) return null;
...
print("✅ تم تحديث دالتي التحويل الهجري/الميلادي في Entitlements.jsx بنجاح (تقويم أم القرى الرسمي)")
